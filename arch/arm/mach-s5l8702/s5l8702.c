...
#define s5l8702_CLKCON ((volatile s5l8702_clkcon *)0x3C500000)

typedef struct {
    uint8_t gate;
    uint8_t bit;
} s5l8702_clkgate_index; 

typedef struct {
    char *id;
    s5l8702_clkgate_index clkgate1;
    // Some of the clockgate mappings have two clockgates. If so, this field will be non-zero.
    s5l8702_clkgate_index clkgate2;
} s5l8702_clkgate_mapping; 

static const s5l8702_clkgate_mapping *s5l8702_clkgate_mappings[] = {
    &(s5l8702_clkgate_mapping) {
...
static void s5l8702_enable_clkgate_bit(uint8_t gate, uint8_t bit) {
    uint32_t mask = ~(((uint32_t) 1) << bit);
    s5l8702_CLKCON->gates[gate] &= mask;
}

static void s5l8702_enable_clkgate(const char *id) {
    s5l8702_clkgate_mapping const **mapping = s5l8702_clkgate_mappings;
    while (mapping != NULL) {
        const s5l8702_clkgate_mapping *m = *mapping;
        if (strcmp(m->id, id) != 0) {
            mapping++;
            continue;
        }

        debug("s5l8702: ungating %s\n", id);
        s5l8702_enable_clkgate_bit(m->clkgate1.gate, m->clkgate1.bit);
        if ((m->clkgate2.gate != 0) && (m->clkgate2.bit != 0)) {
            s5l8702_enable_clkgate_bit(m->clkgate2.gate, m->clkgate2.bit);
        }
        return;
    }
    panic("s5l8702_enable_clkgate: unknown id %s", id);
}
...
static void s5l8702_otgphy_off(void) {
    ...
    volatile struct s5l8702_otgphy *otgphy = (struct s5l8702_otgphy *)0x3c400000;
    otgphy->pwr = 0xff;
    mdelay(10);
    otgphy->rstcon = 0xff;
    mdelay(10);
...
static void s5l8702_otgphy_on(void) {
    ...
    s5l8702_enable_clkgate("usb2-phy");
    mdelay(10);
...
enum s5l8702_timer_cmd {
    s5l8702_TIMER_CMD_STOP = 0,
    s5l8702_TIMER_CMD_START = 1,
    s5l8702_TIMER_CMD_CLR = 2,
};

static struct s5l8702_timer *s5l8702_timer_registers(enum s5l8702_timer_id id) {
    switch (id) {
    case s5l8702_TIMER_A:
        return (struct s5l8702_timer *)0x3c700000;
    case s5l8702_TIMER_B:
        return (struct s5l8702_timer *)0x3c700020;
    case s5l8702_TIMER_C:
        return (struct s5l8702_timer *)0x3c700040;
    case s5l8702_TIMER_D:
        return (struct s5l8702_timer *)0x3c700060;
    case s5l8702_TIMER_E:
        return (struct s5l8702_timer *)0x3c700080;
    case s5l8702_TIMER_F:
        return (struct s5l8702_timer *)0x3c7000a0;
    case s5l8702_TIMER_G:
        return (struct s5l8702_timer *)0x3c7000c0;
    case s5l8702_TIMER_H:
        return (struct s5l8702_timer *)0x3c7000e0;
    case s5l8702_TIMER_I:
        return (struct s5l8702_timer *)0x3c700100;
    default:
        panic("requested invalid timer id %d", id);
    }
}

static const char* s5l8702_timer_clockgate(enum s5l8702_timer_id id) {
    switch (id) {
    case s5l8702_TIMER_A:
        return "timer0";
    case s5l8702_TIMER_B:
...
static void s5l8702_timer_configure_interval(enum s5l8702_timer_id id) {
    debug("s5l8702_timer: configuring %d in interval mode\n", id);
    s5l8702_enable_clkgate(s5l8702_timer_clockgate(id));
    ...
    timer->cmd = s5l8702_TIMER_CMD_CLR;
}

static void s5l8702_timer_start(enum s5l8702_timer_id id) {
    debug("s5l8702_timer: starting %d\n", id);
    volatile struct s5l8702_timer *timer = s5l8702_timer_registers(id);
...
int timer_init(void)
{
    s5l8702_timer_configure_interval(s5l8702_TIMER_F);
    s5l8702_timer_start(s5l8702_TIMER_F);
...
{
    return s5l8702_timer_read(s5l8702_TIMER_F);
}
